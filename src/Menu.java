...
public class Menu {
	...
	public Menu(PApplet drawer, float x, float y, int menuDeck, Card card) {
		this.x = x - MENU_WIDTH / 2;
		this.y = y - MENU_HEIGHT;
		this.type = menuDeck;
		this.card = card;
	...
	public void draw() {
		if (type == MENU_DECK) {
			...
			for(int i = 0; i < NUM; i++) {
				...
				if(i+1 == 2) {
					drawer.fill(255);
					drawer.text("Dip", x+MENU_WIDTH/2, y + i*MENU_HEIGHT/NUM+MENU_HEIGHT/(2*NUM)-SPACE);
				}
				if(i+1 == 3) {
					drawer.fill(255);
					drawer.text("And", x+MENU_WIDTH/2, y + i*MENU_HEIGHT/NUM+MENU_HEIGHT/(2*NUM)-SPACE);
				}
				if(i+1 == 4) {
					drawer.fill(255);
					drawer.text("Rip", x+MENU_WIDTH/2, y + i*MENU_HEIGHT/NUM+MENU_HEIGHT/(2*NUM)-SPACE);
				}
				if(i+1 == 5) {
					drawer.fill(255);
					drawer.text("Other 2", x+MENU_WIDTH/2, y + i*MENU_HEIGHT/NUM+MENU_HEIGHT/(2*NUM)-SPACE);
